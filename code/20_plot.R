...
for (location in attrBurden[, get(agr_by_new)] %>% unique()) {
  for (sourceX in attrBurden$source %>% unique()) {
    ...
    ggsave(file.path(plotDirX, paste0(location, "_plot3.png")), plot = g)

    g <- g + geom_ribbon(aes(ymin = lower, ymax = upper), linetype = 0, alpha = 0.1)
    ggsave(file.path(plotDirX, paste0(location, "_plot3_conf.png")), plot = g)
    ...
    ggsave(file.path(plotDirX, paste0(location, "_plot5.png")), plot = g)
    g <- g + geom_ribbon(aes(ymin = lower, ymax = upper), linetype = 0, alpha = 0.1)
    ggsave(file.path(plotDirX, paste0(location, "_plot5_conf.png")), plot = g)
  }
}
