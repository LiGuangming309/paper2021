...
legend <- legend %>%
  ...
  tidyr::unite("labels", c("Ethnicity", "Education", "rural_urban_class"), sep = "|")

color_legend <- legend$colour
names(color_legend) <- legend$labels
...
t3 <- textGrob("Ethnicity", rot = 90, gp = gpar(fontsize = 10), vjust = 1)
t4 <- textGrob("Education", rot = 90, gp = gpar(fontsize = 10), vjust = 1)
t5 <- textGrob("Rural-Urban", rot = 90, gp = gpar(fontsize = 10), vjust = 1)
t6 <- textGrob("", gp = gpar(fontsize = 10), vjust = 1)
#proportion of all-cause burden attributable

gs <- append(plots, list(t1, t2, legend, t3, t4, t5))
